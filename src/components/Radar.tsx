      ...
      {parte !== 'avvisi' && prossimo && (
        <Card className="border-l-4 border-l-navy">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-5 py-3.5">
            ...
            {prossimo.link && (
              <a href={prossimo.link} target="_blank" rel="noreferrer"
                 className="shrink-0 rounded-[6px] border border-bordo px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.06em] text-navy hover:border-navy">
                Entra
              </a>
            )}
          ...
          {prossimo.preparazione && (
            <div className="border-t border-velo px-5 py-2">
              <button onClick={() => setPreparata(!preparata)}
                ...
                <span className={`inline-block transition-transform ${preparata ? 'rotate-90' : ''}`}>›</span>
